    ...
    <section id="gallery" className="py-16 bg-background">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="font-pixel text-2xl md:text-3xl mb-4 text-primary">STYLE GALLERY</h2>
          <p className="max-w-2xl mx-auto">Check out some of our recent work and get inspired for your next visit.</p>
        </div>
          ...
          {galleryItems.map((item) => (
            <PixelBorder key={item.id} className="overflow-hidden group relative">
              ...
              <div className="absolute inset-0 bg-background bg-opacity-60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                <div className="font-pixel text-primary text-sm">{item.title}</div>
              </div>
            </PixelBorder>
          ))}
        </div>
